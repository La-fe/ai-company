...
> 本文档在 Phase 1-5（逐竞品五层分析）时加载。
> 来源: DDD (Eric Evans), Competitive Intelligence, Product Teardown
> 层级对应: Layer 2 数据模型 / Layer 3 用户旅程 / Layer 4 AI 合约(→ai-contract-framework.md) / Layer 5 Feature+商业
...
> 详细方法论见 `ai-contract-framework.md`。此处仅列出要点。

AI 合约提取不是简单列出"竞品有什么 AI 功能"，而是逆向工程每个 AI 功能的**输入/输出/质量线/降级方案**。

### 快速提取清单

对每个竞品的每个 AI 功能，回答四个问题：

1. **输入**: AI 需要什么才能工作？用户要提供什么？系统能自动获取什么？
2. **输出**: AI 产出什么格式？一次几个？用户能编辑吗？
3. **质量线**: 用户通常需要编辑/重新生成吗？"重新生成"按钮有多显眼？
4. **降级方案**: AI 失败/不可用时用户看到什么？有没有手动替代路径？

### 信息来源优先级

| 来源 | 能提取什么 | 可靠度 |
|------|-----------|--------|
| 亲自试用 AI 功能 | 完整合约四维度 | 最高 |
| 产品演示/教程视频 | 输入+输出+HITL | 高 |
| 用户评价（AI 质量相关） | 质量线+降级体验 | 中 |
| 官网功能介绍 | 输入+输出（宣传版） | 低 |
| 定价页 AI 额度限制 | 限流机制 | 中 |

> `--skip-ai` 或竞品无 AI 功能时，跳过此层。
...
## Feature+商业提取方法（Layer 5）

> Layer 5 合并了 Feature 目录、定价模型、留存机制、设计决策——它们共同描述竞品"卖什么、怎么赚钱、怎么留人"。
...
| **内容沉淀** | 创作内容积累形成壁垒 | 产品、课程、模板 |
| **收入依赖** | 用户已在平台上赚钱 | 订单、客户、收入 |
...
| 层 | 维度 | 最低标准 |
|----|------|---------|
| L1 问题空间 | 用户画像 | 具体到人群特征，非泛称 |
| L1 问题空间 | JTBD | 至少 1 个核心 JTBD + 触发场景 |
| L2 数据模型 | 实体数量 | ≥ 5 个核心实体 |
| L2 数据模型 | 实体分类 | 每个实体标注类型（核心/辅助/价值/事务） |
| L2 数据模型 | 关系标注 | 每个实体至少 1 个标准化关系（使用 `-1:N->` 等符号） |
| L2 数据模型 | 字段定义 | 3-5 个核心实体有字段级别定义 |
| L2 数据模型 | 数据来源 | 每个结论标注来源（API / UI / 定价页 / 推测） |
| L3 用户旅程 | 旅程步骤 | ≥ 5 步（从注册到核心价值） |
| L3 用户旅程 | 流程图 | ≥ 1 个关键流程的 ASCII 流程图 |
| L4 AI 合约 | 合约提取 | 每个 AI 功能有输入/输出/质量线/降级四维度（无 AI 功能则标注） |
| L5 Feature+商业 | Feature 目录 | 按类别列出完整功能清单 |
| L5 Feature+商业 | 定价信息 | 至少覆盖免费层和最低付费层 + 实体边界映射 |
| L5 Feature+商业 | 留存机制 | 至少识别 3 种留存策略 + 涉及实体 |
